...
export function SystemHealthBanner() {
  ...
  const offlineServices = systemHealth.services.filter(
    (s) => s.status === "offline",
  );
  ...
  const getActionText = () => {
    ...
    const workersOffline = offlineServices.some(
      (s) => s.name === "Bull Dashboard",
    );
  ...
  return (
    <div
      className={`border-b ${getBannerStyle()} transition-all duration-300 animate-fade-in`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between py-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            {offlineServices.some(
              (s) => s.name === "Redis" || s.name === "Bull Dashboard",
            ) && (
              <Button
                variant="ghost"
                size="sm"
