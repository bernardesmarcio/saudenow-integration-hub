...
export function SystemHealthIndicator() {
  ...
  const getServiceIcon = (serviceName: string, status: string) => {
    ...
    const iconClass = status === 'online' ? 'text-success-500' : 'text-error-500'
  ...
  const getServiceStatus = (status: string) => {
    switch (status) {
      case 'online':
        return { text: 'Online', class: 'text-success-600 bg-success-50 border-success-200' }
      case 'offline':
        return { text: 'Offline', class: 'text-error-600 bg-error-50 border-error-200' }
      case 'checking':
        return { text: 'Verificando', class: 'text-gray-600 bg-gray-50 border-gray-200' }
      default:
        return { text: 'Desconhecido', class: 'text-gray-600 bg-gray-50 border-gray-200' }
    }
  }

  const onlineServices = systemHealth.services.filter(s => s.status === 'online').length
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center text-lg">
            ...
            <Badge className={`ml-3 ${getOverallColor()} border font-medium capitalize`}>
      ...
      {isExpanded && (
        <CardContent className="pt-0">
            ...
            {systemHealth.services.map((service, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center space-x-2">
                    <Badge className={`${statusInfo.class} border font-medium text-xs`}>
                      {statusInfo.text}
                    </Badge>
                    <Button
                      variant="ghost"
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => window.open('http://localhost:3001/api/v1/retail-pro/health', '_blank')}
                className="text-xs"
              >
          ...
          <div className="mt-3 text-xs text-gray-500 text-center">
            Última atualização: {new Date(systemHealth.lastUpdate).toLocaleString('pt-BR')}
          </div>
        </CardContent>
      )}
    </Card>
