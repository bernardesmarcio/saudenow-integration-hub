...
export default function RetailProPage() {
  ...
  const loadStockData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/v1/retail-pro/stores/${STORE_ID}/stock?${params}`,
      );
  ...
  const handleSync = async (
    type: "incremental_sync" | "stock_sync" | "full_sync",
  ) => {
    try {
      const response = await fetch(
        `/api/v1/retail-pro/stores/${STORE_ID}/sync`,
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">
            Monitoramento em tempo real dos produtos
          </p>
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {syncStatus.products_synced.toLocaleString()}
                </p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">
                ...
                <p className="text-2xl font-bold">
                  {syncStatus.stock_synced.toLocaleString()}
                </p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">
                ...
                <p className="text-sm">
                  {syncStatus.last_stock_sync
                    ? new Date(syncStatus.last_stock_sync).toLocaleString(
                        "pt-BR",
                      )
                    : "Nunca"}
                </p>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {summary.no_data.toLocaleString()}
                  </p>
                </div>
              </div>
            </CardContent>
                ...
                <tbody>
                  {filteredData.map((item) => (
                    ...
                    >
                      ...
                      <td className="p-2 text-center text-muted-foreground">
                        {item.po_ordered_quantity}
                      </td>
                      <td className="p-2 text-center text-muted-foreground">
                        {item.po_received_quantity}
                      </td>
      ...
      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">
          Exibindo {filteredData.length} de {summary?.total_products || 0}{" "}
          produtos
        </p>
        <div className="flex space-x-2">
          <Button
