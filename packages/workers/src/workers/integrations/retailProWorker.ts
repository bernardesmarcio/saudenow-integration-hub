...
import {
  retailProQueue as retailProQueueConfig,
  criticalStockQueue,
  notificationQueue,
} from "../../config/queues";
...
export class RetailProWorker {
  ...
  async process(job: Job<RetailProJobData>) {
    ...
    try {
      ...
      const lockAcquired = await this.retailProService.acquireSyncLock(
        store_sid,
        600,
      ); // 10 minutes
      if (!lockAcquired) {
        this.logger.warn("Could not acquire sync lock, job will be retried", {
          store_sid,
          type,
        });
      ...
      await this.retailProService.updateSyncStatus(store_sid, {
        status: "error",
        errors:
          (await this.retailProService.getSyncStatus(store_sid)).errors + 1,
      });
  ...
  private async syncIncremental(store_sid: string, options: any): Promise<any> {
    ...
    const result = await this.syncStockOnly(store_sid, {
      ...options,
      force: true,
    });
  ...
  private async syncStockOnly(store_sid: string, options: any): Promise<any> {
    ...
    try {
      ...
      this.logger.info(`Processing stock for ${productSids.length} products`, {
        store_sid,
      });

      // Process in batches
      const batchSize =
        options.batch_size || RETAIL_PRO_CONSTANTS.DEFAULT_BATCH_SIZE;
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        const stockEntities = batchResult.success
          .map((stock) =>
            this.transformStockToEntity(
              stock,
              products.find((p) => p.retail_id === stock.store_sid),
            ),
          )
          .filter(Boolean);
        ...
        const criticalStock = batchResult.success.filter(
          (stock) =>
        ...
        const zeroStock = batchResult.success.filter(
          (stock) => stock.status === "out_of_stock" && stock.quantity === 0,
        );
   ...
   */
  private async syncProductsOnly(
    store_sid: string,
    options: any,
  ): Promise<any> {
  ...
  private async syncAllProducts(store_sid: string, options: any): Promise<any> {
    ...
    while (hasMore) {
      try {
        this.logger.debug(`Fetching products batch`, {
          store_sid,
          offset,
          batchSize,
        });
        ...
        this.logger.error("Error in product batch sync", {
          store_sid,
          offset,
          error,
        });
  ...
  private transformProductToEntity(product: any): RetailProProductEntity {
    return {
      ...
      sku: product.alu,
      nome: [product.description1, product.description2]
        .filter(Boolean)
        .join(" ")
        .trim(),
   ...
   */
  private transformStockToEntity(
    stock: ProcessedRetailProStock,
    product?: any,
  ): RetailProEstoqueEntity | null {
    if (!product) {
      return null;
    }
  ...
  private mapStockStatus(
    status: string,
  ): "disponivel" | "indisponivel" | "baixo_estoque" | "sem_dados" {
    switch (status) {
      case "in_stock":
        return "disponivel";
      case "out_of_stock":
   ...
   * Process critical stock alerts
   */
  private async processCriticalStock(
    criticalStock: ProcessedRetailProStock[],
    store_sid: string,
  ): Promise<void> {
    for (const stock of criticalStock) {
      await notificationQueue.add(
        "retail-pro-critical-stock",
        {
          store_sid,
    ...
    this.logger.warn(`Queued ${criticalStock.length} critical stock alerts`, {
      store_sid,
    });
  }
   ...
   * Process zero stock alerts
   */
  private async processZeroStock(
    zeroStock: ProcessedRetailProStock[],
    store_sid: string,
  ): Promise<void> {
    for (const stock of zeroStock) {
      await criticalStockQueue.add(
        "retail-pro-zero-stock",
        {
          store_sid,
          store_name: stock.store_name,
          quantity: stock.quantity,
    ...
    this.logger.error(`Queued ${zeroStock.length} ZERO STOCK alerts`, {
      store_sid,
    });
  }
...
retailProQueueConfig.process(
  "retailpro-sync",
  2,
  async (job: Job<RetailProJobData>) => {
    const worker = new RetailProWorker();
    await worker.process(job);
  },
);

export { retailProQueueConfig as retailProQueue };
