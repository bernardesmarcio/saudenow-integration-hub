...
import { sapEstoqueQueue, criticalStockQueue, notificationQueue } from '../../config/queues';
...
interface SapEstoqueJobData {
  type: 'sync-estoque-delta' | 'sync-estoque-criticos' | 'alert-estoque-zero' | 'preload-popular';
  lastSync?: Date;
  produtoId?: string;
  threshold?: number;
}
...
export class SapEstoqueWorker {
  ...
  async process(job: Job<SapEstoqueJobData>) {
    ...
    try {
      ...
      await logIntegration('sap', `estoque-${type}`, 'error', {
        error: error.message,
        lastSync,
        produtoId,
      }, error.message);
  ...
  private async syncEstoqueDelta(lastSync?: Date): Promise<void> {
    ...
    const transformedEstoques = estoques.map(estoque => this.transformEstoque(estoque));
    ...
    const zeroStockItems = transformedEstoques.filter(e => e.quantidade === 0);
  ...
  private async syncEstoqueCriticos(threshold = 10): Promise<void> {
    ...
    const startTime = Date.now();
    const criticalEstoques = await this.sapEstoqueService.fetchEstoqueCritico(threshold);
  ...
  private async alertEstoqueZero(produtoId: string): Promise<void> {
    ...
    if (!produto) {
      estoqueLogger.error('Product not found for zero stock alert', { produtoId });
      return;
    }

    // Send notification
    await notificationQueue.add('estoque-zero', {
      produtoId,
      produto,
  ...
  private async preloadPopularProducts(): Promise<void> {
    ...
    try {
      ...
      const estoques = await this.sapEstoqueService.fetchEstoqueBatch(produtoIds);
      ...
      estoqueLogger.info(`Preloaded ${estoqueData.length} popular products to cache`);
    } catch (error) {
  ...
  private transformEstoque(estoque: any): any {
    return {
      ...
      quantidade_disponivel: estoque.quantidade_disponivel || estoque.quantidade,
      deposito: estoque.deposito,
      localizacao: estoque.localizacao,
      lote: estoque.lote,
      validade: estoque.validade ? new Date(estoque.validade).toISOString() : null,
  ...
  private async processCriticalStock(criticalItems: any[]): Promise<void> {
    for (const item of criticalItems) {
      await notificationQueue.add('estoque-critico', {
  ...
  private async processZeroStock(zeroStockItems: any[]): Promise<void> {
    for (const item of zeroStockItems) {
      await criticalStockQueue.add('estoque-zero', {
        produtoId: item.produto_id,
        sku: item.sku,
        deposito: item.deposito,
        timestamp: new Date().toISOString(),
