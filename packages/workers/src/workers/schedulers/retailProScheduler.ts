...
export class RetailProScheduler {
  ...
  start(): void {
    ...
    const stockSyncJob = cron.schedule('*/15 * * * *', async () => {
      try {
        ...
        await retailProQueue.add('retailpro-sync', {
          type: 'stock_sync',
          store_sid: storeSid,
          options: {
            batch_size: 100,
    ...
    const incrementalSyncJob = cron.schedule('0 */2 * * *', async () => {
      try {
        const storeSid = RETAIL_PRO_CONSTANTS.STORES.RESENDE.SID
        const lastSync = await getLastSyncTimestamp('retailpro', 'incremental-sync')
    ...
    this.logger.info(`${this.jobs.length} Retail Pro schedulers started successfully`)
  }
  ...
  private async getPopularProducts(storeSid: string): Promise<string[]> {
    // In real implementation, this would query analytics data
    // For now, return mock data
    return Array.from({ length: 50 }, (_, i) => 
      `${(i + 1).toString().padStart(10, '0')}`
    )
  }
  ...
  private async cleanupSyncStatus(): Promise<void> {
    try {
      ...
      if (syncStatus.errors > 0) {
        const lastSync = syncStatus.last_stock_sync || syncStatus.last_product_sync
        if (lastSync) {
  ...
  async triggerManualSync(
    ...
    try {
      ...
      const job = await retailProQueue.add('retailpro-sync', {
        type,
        store_sid: storeSid,
        options: {
      ...
      this.logger.error('Failed to trigger manual sync', { type, priority, error })
      throw error
    }
  }
