...
export class SapScheduler {
  ...
  start(): void {
    ...
    const estoqueJob = cron.schedule('*/2 * * * *', async () => {
      try {
        const lastSync = await getLastSyncTimestamp('sap', 'sync-estoque-delta');
        
        await sapEstoqueQueue.add('sync-estoque-delta', {
    ...
    const estoqueCriticoJob = cron.schedule('*/1 * * * *', async () => {
      try {
        await criticalStockQueue.add('sync-estoque-criticos', {
    ...
    const produtosJob = cron.schedule('*/30 * * * *', async () => {
      try {
        const lastSync = await getLastSyncTimestamp('sap', 'sync-produtos-delta');
        
        await sapSyncQueue.add('sync-produtos-delta', {
    ...
    const clientesJob = cron.schedule('0 * * * *', async () => {
      try {
        const lastSync = await getLastSyncTimestamp('sap', 'sync-clientes-delta');
        
        await sapSyncQueue.add('sync-clientes-delta', {
    ...
    const vendasJob = cron.schedule('*/10 * * * *', async () => {
      try {
        ...
        await sapSyncQueue.add('sync-vendas-delta', {
    ...
    const fullSyncJob = cron.schedule('0 2 * * *', async () => {
      try {
        await sapSyncQueue.add('full-sync', {
          type: 'full-sync',
          force: true,
        }, {
          priority: 1,
    ...
    const preloadJob = cron.schedule('0 */6 * * *', async () => {
      try {
        await sapEstoqueQueue.add('preload-popular', {
    ...
    workerLogger.info(`${this.jobs.length} SAP schedulers started successfully`);
  }
