...
export class RetailProService extends BaseIntegrationService {
  ...
  async getProducts(
    ...
    try {
      ...
      // Try cache first
      const cached =
        await CacheManager.get<RetailProApiResponse<RetailProProduct>>(
          cacheKey,
        );
      if (cached) {
      ...
      const queryParams = {
        cols:
          options.cols ||
          "sid,alu,description1,description2,vendor_name,sbsinventoryqtys",
        limit: options.limit || RETAIL_PRO_CONSTANTS.DEFAULT_BATCH_SIZE,
        offset: options.offset || 0,
        ...options,
      };

      this.logger.info("Fetching products from Retail Pro", {
        storeSid,
        queryParams,
      });

      const response = await this.get<RetailProProduct[]>(
        "/v1/rest/inventory",
        {
          params: queryParams,
        },
      ...
      await CacheManager.set(
        cacheKey,
        result,
        RETAIL_PRO_CONSTANTS.CACHE_TTL.PRODUCTS,
      );
  ...
  async getProductStock(
    inventorySid: string,
    storeSid: string,
    options: Partial<RetailProStockQuery> = {},
    ...
    try {
      ...
      const queryParams = {
        cols:
          options.cols ||
          "store_sid,store_name,quantity,minimum_quantity,po_ordered_quantity,po_received_quantity",
        ...options,
      };

      this.logger.debug("Fetching stock from Retail Pro", {
        inventorySid,
        storeSid,
        queryParams,
      });
      ...
      if (stock) {
        await CacheManager.set(
          cacheKey,
          stock,
          RETAIL_PRO_CONSTANTS.CACHE_TTL.STOCK,
        );
      }

      this.logger.debug("Stock fetched successfully", {
        inventorySid,
        storeSid,
        hasStock: !!stock,
      });
      ...
      this.logger.error("Error fetching stock", {
        inventorySid,
        storeSid,
        error,
      });
  ...
  async getProductsStockBatch(
    ...
    const batchSize = Math.min(
      inventorySids.length,
      this.config.batchSize || RETAIL_PRO_CONSTANTS.DEFAULT_BATCH_SIZE,
    );
  ...
  private processStock(
    ...
    if (stock.quantity > 0) {
      status =
        stock.quantity <= stock.minimum_quantity ? "low_stock" : "in_stock";
    }
  ...
  async getSyncStatus(storeSid: string): Promise<RetailProSyncStatus> {
    ...
    await CacheManager.set(
      cacheKey,
      defaultStatus,
      RETAIL_PRO_CONSTANTS.CACHE_TTL.SYNC_STATUS,
    );
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      const testResponse = await this.get<RetailProProduct[]>(
        "/v1/rest/inventory",
        {
          params: { limit: 1 },
          timeout: 10000,
  ...
  protected transformToInternal(
    data: RetailProProduct,
  ): ProcessedRetailProProduct {
    return this.processProduct(data);
  }

  protected transformToExternal(
    data: ProcessedRetailProProduct,
  ): RetailProProduct {
    return {
      sid: data.sid,
      alu: data.alu,
  ...
  private handleError(
    ...
    if (error.response?.status === 404) {
      return new RetailProNotFoundError(
        "Resource",
        productSid || storeSid || "unknown",
      );
    }
