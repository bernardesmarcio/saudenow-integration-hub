...
export class SapWebhookServer {
  ...
  private setupRoutes(): void {
    ...
    // SAP Stock webhook
    this.app.post('/webhooks/sap/estoque', this.verifySignature.bind(this), this.handleEstoqueWebhook.bind(this));

    // SAP Products webhook
    this.app.post('/webhooks/sap/produtos', this.verifySignature.bind(this), this.handleProdutosWebhook.bind(this));
  ...
  private verifySignature(req: express.Request, res: express.Response, next: express.NextFunction): void {
  ...
  private async handleEstoqueWebhook(req: express.Request, res: express.Response): Promise<void> {
    try {
  ...
  private async handleProdutosWebhook(req: express.Request, res: express.Response): Promise<void> {
    try {
      const { event, data } = req.body;
  ...
  private async processStockUpdate(data: any[]): Promise<void> {
    ...
    for (const update of updates) {
      ...
      await sapEstoqueQueue.add('webhook-stock-update', {
  ...
  private async processStockDepletion(data: any[]): Promise<void> {
    ...
    for (const depletion of depletions) {
      ...
      await criticalStockQueue.add('webhook-stock-zero', {
  ...
  private async processStockCritical(data: any[]): Promise<void> {
    ...
    for (const critical of criticals) {
      ...
      // Queue critical sync
      await criticalStockQueue.add('webhook-stock-critical', {
  ...
  private async processProductUpdate(data: any[]): Promise<void> {
    ...
    for (const update of updates) {
      ...
      // Queue product sync
      await sapEstoqueQueue.add('webhook-product-update', {
    ...
    return new Promise((resolve, reject) => {
      try {
        this.server = this.app.listen(port, () => {
          ...
          workerLogger.info(`   • POST http://localhost:${port}/webhooks/sap/estoque`);
          workerLogger.info(`   • POST http://localhost:${port}/webhooks/sap/produtos`);
        ...
        this.server.on('error', (error: any) => {
          if (error.code === 'EADDRINUSE') {
            workerLogger.warn(`Port ${port} is in use, webhook server disabled`);
