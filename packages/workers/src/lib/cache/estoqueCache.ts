...
export class EstoqueCache {
  ...
  async getMultiple(produtoIds: string[]): Promise<Map<string, EstoqueData>> {
    ...
    try {
      ...
      cacheLogger.debug(
        `Cache multi-get: ${result.size}/${produtoIds.length} hits`,
      );
    } catch (error) {
  ...
  async preloadPopular(
    produtos: Array<{ id: string; estoque: EstoqueData }>,
  ): Promise<void> {
  ...
  async clearAll(): Promise<void> {
    try {
      const patterns = [`${this.prefix}*`, `${this.criticalPrefix}*`];
