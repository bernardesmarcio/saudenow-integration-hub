...
export class RetailProCache {
  ...
  async cacheProduct(
    ...
    try {
      ...
      this.logger.error("Error caching product", {
        storeSid,
        productSid,
        error,
      });
  ...
  async getProduct(
    ...
    try {
      ...
      this.logger.error("Error getting cached product", {
        storeSid,
        productSid,
        error,
      });
  ...
  async cacheProducts(
    ...
    try {
      const promises = Array.from(products.entries()).map(
        ([productSid, product]) =>
  ...
  async cacheStock(
    ...
    try {
      ...
      this.logger.debug("Stock cached", {
        storeSid,
        productSid,
  ...
  async getStock(
    ...
    try {
      ...
      this.logger.error("Error getting cached stock", {
        storeSid,
        productSid,
        error,
      });
  ...
  async cacheStockBatch(
    ...
    try {
      const promises = Array.from(stockMap.entries()).map(
        ([productSid, stock]) => this.cacheStock(storeSid, productSid, stock),
      );
  ...
  async cacheSyncStatus(
    ...
    try {
      ...
      this.logger.debug("Sync status cached", {
        storeSid,
  ...
  async getSyncStatus(storeSid: string): Promise<RetailProSyncStatus | null> {
    try {
      ...
      this.logger.error("Error getting cached sync status", {
        storeSid,
        error,
      });
      return null;
   ...
   */
  async cacheStoreConfig(storeSid: string, config: any): Promise<void> {
    try {
      const key = RETAIL_PRO_CACHE_KEYS.STORE_CONFIG(storeSid);
      await CacheManager.set(
        key,
        config,
        RETAIL_PRO_CONSTANTS.CACHE_TTL.CONFIG,
      );
  ...
  async getStoreConfig(storeSid: string): Promise<any | null> {
    try {
      ...
      this.logger.error("Error getting cached store config", {
        storeSid,
        error,
      });
      return null;
  ...
  async invalidateStoreStock(storeSid: string): Promise<void> {
    try {
      ...
      this.logger.error("Error invalidating store stock cache", {
        storeSid,
        error,
      });
    }
  ...
  async invalidateStoreProducts(storeSid: string): Promise<void> {
    try {
      ...
      this.logger.error("Error invalidating store products cache", {
        storeSid,
        error,
      });
    }
  ...
  async preloadPopularProducts(
    ...
    try {
      ...
      const stockPromises = productData
        .filter(({ stock }) => stock)
        .map(({ sid, stock }) => this.cacheStock(storeSid, sid, stock!));
      ...
      this.logger.error("Error preloading popular products", {
        storeSid,
        error,
      });
    }
