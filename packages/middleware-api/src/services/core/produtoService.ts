...
import { DevProduto, DevProdutoInsert, DevProdutoUpdate } from '../../lib/database/types';
import {
  ProdutoResponse,
...
export class ProdutoService {
  ...
  private mapToUpdate(request: ProdutoUpdateRequest): DevProdutoUpdate {
    ...
    if (request.shopify_id !== undefined) update.shopify_id = request.shopify_id;
  ...
  async list(query: ProdutoListQuery): Promise<{ produtos: ProdutoResponse[]; total: number }> {
    try {
      const {
        page = 1,
        limit = 20,
      ...
      const produtos = await db.select<DevProduto>(Tables.PRODUTOS, selectQuery, filters);
      ...
      if (preco_min !== undefined) {
        filteredProdutos = filteredProdutos.filter(produto =>
          produto.preco !== null && produto.preco >= preco_min
        );
      }

      if (preco_max !== undefined) {
        filteredProdutos = filteredProdutos.filter(produto =>
          produto.preco !== null && produto.preco <= preco_max
        );
      }
      ...
      return {
        produtos: paginatedProdutos.map(produto => this.mapToResponse(produto)),
        total,
      };
    } catch (error) {
  ...
  async getById(id: string): Promise<ProdutoResponse | null> {
    try {
      const produtos = await db.select<DevProduto>(Tables.PRODUTOS, '*', { id });
  ...
  async getBySku(sku: string): Promise<ProdutoResponse | null> {
    try {
      const produtos = await db.select<DevProduto>(Tables.PRODUTOS, '*', { sku });
  ...
  // Update product
  async update(id: string, request: ProdutoUpdateRequest): Promise<ProdutoResponse> {
    try {
      ...
      const produto = await db.update<DevProduto>(Tables.PRODUTOS, id, updateData);
  ...
  async bulkCreate(requests: ProdutoCreateRequest[]): Promise<BulkOperationResult> {
  ...
  async getByExternalId(system: 'sap' | 'crm' | 'shopify' | 'retail', externalId: string): Promise<ProdutoResponse[]> {
    try {
      const fieldMap = {
        sap: 'sap_id',
        crm: 'crm_id',
      ...
      const produtos = await db.select<DevProduto>(Tables.PRODUTOS, '*', filters);
