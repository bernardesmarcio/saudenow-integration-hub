...
// Types for mock request/response with node-mocks-http methods
type ApiRequest = MockRequest<NextApiRequest>;
type ApiResponse = MockResponse<NextApiResponse> & {
  _getStatusCode(): number;
  _getData(): string;
};

// Helper to create typed mocks for Next.js API routes
const createApiMocks = (options: Parameters<typeof createMocks>[0]) => {
