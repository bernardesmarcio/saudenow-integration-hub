...
describe('/api/v1/produtos', () => {
  ...
  describe('GET /api/v1/produtos', () => {
    ...
    it('should handle service errors gracefully', async () => {
      mockProdutoService.list.mockRejectedValue(new Error('Database connection failed'));
  ...
  describe('POST /api/v1/produtos', () => {
    ...
    it('should return 409 for duplicate SKU', async () => {
      mockProdutoService.create.mockRejectedValue(new Error('Produto com SKU \'PRD-TEST\' já existe'));
...
describe('Product validation edge cases', () => {
  ...
  it('should validate SKU format correctly', async () => {
    ...
    expect(response.success).toBe(false);
    expect(response.details).toContain(expect.stringMatching(/SKU deve conter apenas letras maiúsculas/));
  });
  ...
  it('should validate price and cost relationship', async () => {
    const invalidPriceData = {
      ...
      nome: 'Produto de Teste',
      preco: 5.00,
      custo: 10.00, // Cost higher than price
    };
    ...
    expect(response.success).toBe(false);
    expect(response.details).toContain(expect.stringMatching(/Custo não pode ser maior que o preço/));
  });
  ...
  it('should validate image URLs', async () => {
    ...
    expect(response.success).toBe(false);
    expect(response.details).toContain(expect.stringMatching(/URL da imagem deve ser válida/));
  });
});