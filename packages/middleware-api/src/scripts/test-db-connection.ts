...
async function testDatabaseConnection() {
  log(
    '\n🔍 Testing Supabase Database Connection for SaúdeNow Integration Hub',
  ...
  try {
    ...
    try {
      const insertedProduct = await db.insert<DevProduto>(
        Tables.PRODUTOS,
        testProduct
      )
      log(
        `✅ Product inserted successfully: ${insertedProduct.id} (${insertedProduct.sku})`,
        colors.green
      )

      // Test 4: Read the inserted product
      log('\n4. Testing product retrieval...', colors.yellow)
      const products = await db.select<DevProduto>(Tables.PRODUTOS, '*', {
        sku: insertedProduct.sku,
      })
      if (products.length > 0) {
        log(
          `✅ Product retrieved successfully: ${products[0].nome}`,
          colors.green
        )
        log(
          `   Details: ${products[0].categoria} - R$ ${products[0].preco}`,
          colors.reset
        )
      } else {
      ...
      const updatedProduct = await db.update<DevProduto>(
        Tables.PRODUTOS,
        insertedProduct.id,
        {
      ...
      log(
        `✅ Product updated successfully: ${updatedProduct.nome} - R$ ${updatedProduct.preco}`,
        colors.green
      )
    ...
    log(
      `✅ Table name resolved: ${Tables.PRODUTOS} -> ${tableName}`,
      colors.green
    )
    ...
    try {
      ...
      log(
        `✅ Admin client works: ${adminStats.count} products accessible`,
        colors.green
      )
    } catch (error) {
    ...
    log(
      '1. Configure your .cursor-settings with your Supabase PROJECT_REF and ACCESS_TOKEN',
      colors.reset
    )
    log(
      '2. Run the migration: Run the SQL in database/supabase/migrations/001_dev_schema.sql in your Supabase dashboard',
      colors.reset
    )
    log(
      '3. Run the seeds: Run the SQL in database/supabase/seeds/001_dev_data.sql in your Supabase dashboard',
      colors.reset
    )
