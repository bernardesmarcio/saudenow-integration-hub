...
export const ProductCreateSchema = z.object({
  sku: z
    ...
    .regex(/^[A-Z0-9_-]+$/, 'SKU deve conter apenas letras maiúsculas, números, hífens e underscores'),
  nome: z
    .string()
  ...
  imagens: z
    .array(
      z.string().url('URL da imagem deve ser válida')
    )
    .max(10, 'Máximo 10 imagens permitidas')
    .optional(),
  ativo: z.boolean().optional().default(true),
  sap_id: z.string().max(50, 'SAP ID deve ter no máximo 50 caracteres').optional(),
  crm_id: z.string().max(50, 'CRM ID deve ter no máximo 50 caracteres').optional(),
  shopify_id: z.string().max(50, 'Shopify ID deve ter no máximo 50 caracteres').optional(),
  retail_id: z.string().max(50, 'Retail ID deve ter no máximo 50 caracteres').optional(),
...
export const ProductListQuerySchema = z.object({
  ...
  preco_min: z
    ...
    .refine((val) => val === undefined || val >= 0, 'Preço mínimo deve ser maior ou igual a zero'),
  preco_max: z
    .string()
    .optional()
    .transform((val) => val ? parseFloat(val) : undefined)
    .refine((val) => val === undefined || val >= 0, 'Preço máximo deve ser maior ou igual a zero'),
}).refine(
  (data) => {
...
export function validateProductCreate(data: unknown): { success: true; data: ProductCreateInput } | { success: false; errors: string[] } {
  try {
...
export function validateProductUpdate(data: unknown): { success: true; data: ProductUpdateInput } | { success: false; errors: string[] } {
  try {
...
export function validateProductListQuery(query: unknown): { success: true; data: ProductListQuery } | { success: false; errors: string[] } {
  try {
...
export function validateUUID(id: unknown): { success: true; data: string } | { success: false; errors: string[] } {
  try {
...
export function validateProductBulkCreate(data: unknown): { success: true; data: ProductBulkCreateInput } | { success: false; errors: string[] } {
  try {
