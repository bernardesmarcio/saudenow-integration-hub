    ...
    run: async () => {
      try {
        ...
        if (response.statusCode === 200) {
          log(
            `✓ GET /api/v1/produtos - Status: ${response.statusCode}`,
            colors.green,
          );
          ...
          log(
            `  Erro: ${response.data.error || "Erro desconhecido"}`,
            colors.red,
          );
    ...
    run: async () => {
      try {
        ...
        if (response.statusCode === 201) {
          log(
            `✓ POST /api/v1/produtos - Status: ${response.statusCode}`,
            colors.green,
          );
          log(
            `  Produto criado: ${response.data.data.nome} (ID: ${response.data.data.id})`,
            colors.cyan,
          );
  ...
  },
  {
    ...
    run: async (context) => {
      ...
      try {
        const response = await makeRequest(
          "GET",
          `/api/v1/produtos/${context.productId}`,
        );

        if (response.statusCode === 200) {
          log(
            `✓ GET /api/v1/produtos/${context.productId} - Status: ${response.statusCode}`,
            colors.green,
          );
  ...
  },
  {
    ...
    run: async (context) => {
      ...
      try {
        ...
        if (response.statusCode === 200) {
          log(
            `✓ PUT /api/v1/produtos/${context.productId} - Status: ${response.statusCode}`,
            colors.green,
          );
  ...
  },
  {
    ...
    run: async () => {
      ...
      try {
        ...
        if (response.statusCode === 200) {
          log(
            `✓ POST /api/v1/produtos/bulk - Status: ${response.statusCode}`,
            colors.green,
          );
          log(
            `  Processados: ${response.data.data.total}, Sucessos: ${response.data.data.success}, Falhas: ${response.data.data.failed}`,
            colors.cyan,
          );
  ...
  },
  {
    ...
    run: async () => {
      try {
        const response = await makeRequest(
          "GET",
          "/api/v1/produtos?categoria=Teste&search=Produto&limit=10",
        );

        if (response.statusCode === 200) {
          log(
            `✓ GET /api/v1/produtos (com filtros) - Status: ${response.statusCode}`,
            colors.green,
          );
          log(`  Produtos encontrados: ${response.data.count}`, colors.cyan);
          log(
            `  Página: ${response.data.page}, Limite: ${response.data.limit}`,
            colors.cyan,
          );
          return { success: true };
        } else {
    ...
    run: async (context) => {
      ...
      try {
        ...
        if (response.statusCode === 204) {
          log(
            `✓ DELETE /api/v1/produtos/${context.productId} - Status: ${response.statusCode}`,
            colors.green,
          );
    ...
    run: async (context) => {
      ...
      try {
        ...
        if (response.statusCode === 404) {
          log(
            `✓ GET /api/v1/produtos/${context.productId} - Status: ${response.statusCode} (esperado)`,
            colors.green,
          );
          ...
          log(
            `✗ GET /api/v1/produtos/${context.productId} - Status: ${response.statusCode} (produto ainda existe)`,
            colors.red,
          );
          return { success: false };
        }
    ...
    run: async () => {
      ...
      try {
        ...
        if (response.statusCode === 400) {
          log(
            `✓ POST /api/v1/produtos (dados inválidos) - Status: ${response.statusCode} (esperado)`,
            colors.green,
          );
          ...
          log(
            `✗ POST /api/v1/produtos (dados inválidos) - Status: ${response.statusCode} (deveria ser 400)`,
            colors.red,
          );
          return { success: false };
        }
    ...
    try {
      ...
      if (result && result.success) {
        ...
        if (result.initialCount !== undefined)
          context.initialCount = result.initialCount;
      } else {
        results.failed++;
